...
export function setupUniversal(app: Express, ngOptions: AngularUniversalOptions) {
  ...
  app.engine('html', (_, opts, callback) => {
    ...
    ngOptions.ngExpressEngine({
      ...
      providers: [
        {
          provide: 'serverUrl',
          useValue: `${(options['req'] as Request).protocol}://${(options['req'] as Request).get('host')}`,
        },
...
export function getCacheOptions(ngOptions: AngularUniversalOptions): CacheOptions {
  if (!ngOptions.cache) {
    return {
      isEnabled: false,
    };
